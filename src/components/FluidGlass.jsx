...
export default function FluidGlass({
  ...
  const canvasStyle = overlay
    ? { background: 'transparent', pointerEvents: 'none', width: '100%', height: '100%' }
  ...
  return (
    ...
    >
      <Suspense fallback={null}>
        {overlay ? (
          <Wrapper modeProps={modeProps} backgroundColor={backgroundColor} overlay>
            <StaticImages />
          </Wrapper>
        ) : (
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useFrame((state, delta) => {
    ...
    const px = overlay ? pointerNDC.current.x : pointer.x;
    const py = overlay ? pointerNDC.current.y : pointer.y;
      ...
      {createPortal(
        <>
          ...
          {overlay && (
            <mesh position={[0, 0, -5]} scale={[vp.width * 2, vp.height * 2, 1]}>
              <planeGeometry />
              <meshBasicMaterial color="#1a1030" toneMapped={false} />
            </mesh>
          )}
          {children}
        </>,
...
function StaticImages() {
  ...
  return (
    <group>
      <Image position={[-2, 0, 0]} scale={[3, height / 1.1, 1]} url={IMAGE_URLS[0]} />
      <Image position={[2, 0, 3]} scale={3} url={IMAGE_URLS[1]} />
      <Image position={[-2.05, -height * 0.15, 6]} scale={[1, 3, 1]} url={IMAGE_URLS[2]} />
      <Image position={[-0.6, -height * 0.1, 9]} scale={[1, 2, 1]} url={IMAGE_URLS[3]} />
      <Image position={[0.75, -height * 0.08, 10.5]} scale={1.5} url={IMAGE_URLS[4]} />
    </group>
  );
}
